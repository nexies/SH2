...
QMap<QString, QVariant> ISqlTableItemPrivate::values() const
{
    QMap<QString, QVariant> out;
    auto mobj = this->metaObject();
    for(int i = mobj->propertyOffset(); i < mobj->propertyCount(); i++)
    {
        out.insert(mobj->property(i).name(), mobj->property(i).read(this));
    }
    return out;
}
...
QString ISqlTableItemPrivate::sqlValue(const QString &name) const
{
    ...
    {
        return QString("'\\x%1'").arg(QString(var.toByteArray().toHex()));
    }
    case QVariant::Int:
    {
...
QMap<QString, QVariant> ISqlTableItem::sqlValues() const
{
    return _private->values();
}
