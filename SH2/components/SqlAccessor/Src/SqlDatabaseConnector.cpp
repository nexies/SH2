...
void SqlDatabaseConnector::sendQuery(QueryRequest request)
{
    if(debug) qDebug() << m_state;
    if(_queue.isEmpty() && m_state == Idle)
        emit sendQuerySignal(request);
    else
    {
        qDebug().noquote() << Title << "Queuing query" << request.uuid.toString().mid(1, 36);
...
bool SqlDatabaseConnector::connectToBase(const QString &host, int port, const QString &baseName, const QString &username, const QString &password)
{
    ...
    _queryThread = new SqlQueryThread(_database, this);

    connect(_queryThread, &SqlQueryThread::finished,
            this, &SqlDatabaseConnector::onQueryFinished);
    connect(_queryThread, &SqlQueryThread::result,
            this, &SqlDatabaseConnector::onQueryResult);
...
void SqlDatabaseConnector::onSendQuery(QueryRequest request)
{
    if(!_queryThread)
        return;
    ...
    if(_queryThread->isRunning()) // meaning thread is busy now
    {
        _queue.append(request);
        return;
    }

    _queryThread->exec(request);
    /*
    if(!_database.isOpen())
    {
    ...
    {
        if(debug) qDebug().noquote() << Title << "Executed query" << query_str;
    }
...
void SqlDatabaseConnector::onQueryFinished()
{
    if(!_queue.isEmpty())
    {
        auto q = _queue.dequeue();
//        if (debug) qDebug().noquote() << Title << "Dequeuing query" << uuid.toString().mid(1, 36);
        emit sendQuerySignal(q);
    }
}

void SqlDatabaseConnector::onQueryResult(QueryResult res)
{
    emit queryFinishedSignal(res.requestUuid, res);
}
...
QTextCodec * SqlDatabaseConnector::codec() const
{
    return _codec;
}
